...
### Flag to turn on logging and pdf creation
  ...
  for( iIndex in 1:ncol( dfData ) ){
	...
	#Measure expectation of the feature without zeros
	#vdExp = c( vdExp, mean( vdCur ) )
  ...
  if( fVerbose ){
    ...
    print(paste("Intercept=", dInterceptZero, ", Beta=", dBetaZero, sep=""))
    ...
    # Percent zero
    if( !is.na( dBetaZero ) ){
      print(c(dBetaZero,dInterceptZero))
...
### If true, plotting and logging occur
  ...
  if( iNumAssociations > 0 ){
    ...
    viCorrRangeBugsIdx  = funcSample( viAvailableIndices,iNumAssociations,replace=FALSE)               # The indices of the bugs in the range
    ...
    viNumCorrDomainBugs = funcSample( seq(1,iMaxNumberCorrDomainBugs),
    ...
    for(k in 1:iNumAssociations){
      if( length(viAvailableIndices) >= viNumCorrDomainBugs[k] ){                     # Check that the number of remaining indices is sufficient
         liCorrDomainBugsIdx[[k]] = funcSample( viAvailableIndices,
...
### Controls the plotting of graphic pdf logging (Default FALSE, TRUE indicates logging occurs)
  ...
  # if all vdExp are NAs, not print
  if(fVerbose & ! all(is.na(vdExp)))
  {
...
### The minimum percent of samples a level can have
  ...
  {
    vsCurMetadata = funcSample( binary_names, size=int_number_samples, prob=probability, replace = TRUE)
...
### Turns on logging (typically generates pdfs)
  ...
  write.table( vdExp, "inspection.txt", row.names=F, col.names=F )
...
fVerbose = FALSE
  ...
  #TODO new hack code
#  print("viRemainingFeatures 1")
#  print(viRemainingFeatures)
#  if( FALSE )
    ...
    for(iSpikedBug in 1:iSpikeinCount)
    {
      ...
      while(fSpikeInFailed)
      {
        ...
        if(!is.null(lviFrozeMetadataIndices) & length(lviFrozeMetadataIndices)>0)
        {
          ...
          viSelectedMetadata = funcSample( 1:nrow( metadata_matrix ), multivariate_parameter, replace = TRUE )
        }
  ...
  if(fDummyData)
  {
    ...
    for(iRowIndex in 1:nrow(vdCurMetadata))
    {
      ...
      if(!sum(sapply(vxMetadata,funcNumericIsInt))==0)
      {
        ...
        strLevel = funcSample( vsLevels[ 2:length( vsLevels ) ], size=1 )
...
### The target sum to update the distribution to
  ...
  if(dCurReadDepthDifference > 0)
  {
    ...
    viSamplingIndices = funcSample( x = 1:length( vdDistribution ), size = abs( dCurReadDepthDifference ), replace = TRUE, prob = vdProbabilities )
    # Update indices
...
option_list = list(
  ...
  make_option( c("-i","--spikeCount"), type = "character", default = "1", help = "Counts of spiked metadata used in the spike-in dataset. These values should be comma delimited values, in the order of the spikeStrength values (if given). Can be one value, in this case the value will be repeated to pair with the spikeCount values (if multiple are present). Example 1,2,3"),
  make_option( c("-r","--spikeStrength"), type = "character", default = "1.0", help = "Strength of the metadata association with the spiked-in feature. These values should be comma delimited and in the order of the spikeCount values (if given). Can be one value, in this case the value wil be repeated to pair with the spikeStrength values (if multiple are present). Example 0.2,0.3,0.4."),
  ...
  make_option(c("-u","--minOccurence"), type="integer", default=0, help="Minimum counts a bug can have for the ocurrence quality control filter used when creating bugs. ( Filtering minimum number of counts in a minimum number of samples). A positive integer is expected."),
  ...
  make_option(c("-w","--minSample"), type="integer", default=0, help="Minimum samples a bug can be in for the ocurrence quality control filter used when creating bugs. ( Filtering minimum number of counts in a minimum number of samples). A positive integer is expected."),
  make_option(c("-x","--scalePercentZeros"), type="double", default=1, help="A scale used to multiply the percent zeros of all features across the sample after it is derived from the relatiohships with it and the feature abundance or calibration file. Requires a number greater than 0. A number greater than 1 increases sparsity, a number less than 1 decreases sparsity. O removes sparsity, 1 (default) does not change the value and the value."),
