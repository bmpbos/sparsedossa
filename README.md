...
(1) estimation of model parameters for a null matrix; (2) generation of the null matrix; and (3) spiking associations into the null matrix. Below, we introduce the terminology used throughout the remainder of this section, and then describe these methodological steps in detail. A feature matrix with M microbial community features (represented as rows) and N samples (represented as columns) is denoted as Y and each of its entries take an integer (count-like) value; i.e. Y_(m,n)∈N. The row corresponding to feature i is represented as Y_(i,⋅); similarly, the column corresponding to sample j is as Y_(⋅,j). A null feature matrix Y^((0))is defined as a feature matrix that lacks statistically significant correlation structure (i) among its component features and (ii, optionally) with a given metadata matrix. A metadata matrix is a feature matrix with N columns (corresponding to the N samples from the null matrix) and P rows corresponding to metadata features; metadata features may be continuously valued or categorical. A final spiked feature matrix Y^((1)) is generated by inducing correlation structure among the rows of the null matrix or between rows of the null matrix and metadata features.
